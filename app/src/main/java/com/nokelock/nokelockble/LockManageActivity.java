...
public class LockManageActivity extends MPermissionsActivity implements View.OnClickListener {
    ...
    private void parseData(String value) {
        ...
        }else if (decryptString.startsWith("0703")){
            if (decryptString.startsWith("07030101")){
                deviceCz.setText("当前操作：修改密钥失败");
            }else {
                deviceCz.setText("当前操作：修改密钥成功");
            }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.bt_close://复位
                ...
                break;
            case R.id.bt_update_password://修改密钥和密码

                progressDialog = ProgressDialog.show(context, null, "正在修改密钥...");

                App.getInstance().getBluetoothLeService().writeCharacteristic(new byte[]{0x07, 0x01, 0x08, SampleGattAttributes.newkey[0],SampleGattAttributes.newkey[1],SampleGattAttributes.newkey[2],SampleGattAttributes.newkey[3],SampleGattAttributes.newkey[4],SampleGattAttributes.newkey[5],SampleGattAttributes.newkey[6],SampleGattAttributes.newkey[7], 0x00, 0x00, 0x00, 0x00, 0x00}, SampleGattAttributes.key);

                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        App.getInstance().getBluetoothLeService().writeCharacteristic(new byte[]{0x07, 0x02, 0x08, SampleGattAttributes.newkey[8],SampleGattAttributes.newkey[9],SampleGattAttributes.newkey[10],SampleGattAttributes.newkey[11],SampleGattAttributes.newkey[12],SampleGattAttributes.newkey[13],SampleGattAttributes.newkey[14],SampleGattAttributes.newkey[15], 0x00, 0x00, 0x00, 0x00, 0x00}, SampleGattAttributes.key);
...
//                        progressDialog.setTitle("正在修改密码...");
                        ...
                        handler.postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                App.getInstance().getBluetoothLeService().writeCharacteristic(new byte[]{0x05, 0x04, 0x06,SampleGattAttributes.passwordnew[0],SampleGattAttributes.passwordnew[1],SampleGattAttributes.passwordnew[2],SampleGattAttributes.passwordnew[3],SampleGattAttributes.passwordnew[4],SampleGattAttributes.passwordnew[5],  token[0], token[1], token[2], token[3], 0x00, 0x00, 0x00}, SampleGattAttributes.newkey);

                                progressDialog.dismiss();

                                SampleGattAttributes.key = SampleGattAttributes.newkey;
                                SampleGattAttributes.password = SampleGattAttributes.passwordnew;
                            }
                        }, 2000);
